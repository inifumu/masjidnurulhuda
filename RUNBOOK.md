...
Baseline R3 terakhir yang sudah berada di `testing` tetap commit `04fca5d`. Setelah itu corrective R3 telah dirombak lokal ke shell shadcn-vue Sidebar canonical: grouped submenu, desktop icon collapse, mobile off-canvas, geometry desktop 32 px / mobile 44 px / account 48 px, motion collapse sinkron, nested Escape sheet/account, popup collision aman, serta role matrix `superadmin|ketua|bendahara|pengurus` lulus pada browser lokal. Final gate lokal terbaru lulus (`npm run test` 145/145, `npm run build`, `npm run test:e2e:browser`, custom R3 browser matrix, dan `git diff --check`). Branch/resource production tetap tidak berubah. Promosi testing baru sengaja ditahan sampai cleanup staging, verdict review independen final, dan commit terbaru selesai.
