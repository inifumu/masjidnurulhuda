...
- `src/services/admin/kasService.ts` — `getMasterData`, `getTransactions`, `submitDirectTransaction`, `submitProposal`, `approveTransaction`, `rejectTransaction`, `voidTransaction`, `getTransactionTimeline` — API client ke endpoint transaksi kas yang dipisah per intent, termasuk query periodik `month/year` + filter optional `tipe/kategori_id` untuk list.
