...
- `[Vue] FinanceV2 -> DirectTransactionView -> DirectCashDesk(useKas.handleDirectInput) -> [Vue] Entry Spine Catat kas (arus kas -> nominal -> identitas transaksi -> keterangan/seksi opsional -> closure review; review final hanya di Dialog) -> [Vue] kasService.submitDirectTransaction -> [Vue] httpClient (ApiError on non-2xx) -> [Hono] POST /api/admin/transaction/add-direct -> shared parser + validasi FK seksi (`existsSeksiById`) -> txService.createTransaction -> DB kas_masjid (keperluan dan keterangan tersimpan terpisah; status dipaksa approved; seksi_id opsional)`
- `[Vue] FinanceV2 -> ProposalsView -> KasProposal(useKas.handleProposal) -> [Vue] Request Brief canonical (keperluan-first, arus + nominal + rincian, metadata routing non-sticky dengan consequence + CTA tunggal di akhir; review final hanya Dialog) -> [Vue] parseInputRupiah/formatInputRupiah -> kasService.submitProposal -> httpClient -> [Hono] POST /api/admin/transaction/add-proposal -> validasi DTO + validasi FK seksi (`existsSeksiById`) -> txService.createTransaction -> DB kas_masjid (status awal proposal = pending_ketua, seksi_id wajib dan valid)`
...
- `[Vue] TransactionsView memilih transaksi -> request-sequenced kasService.getTransactionTimeline -> [Hono] GET /api/admin/transaction/:id/timeline -> txService.getTransactionAuditTimeline -> ownership policy pengurus (owner atau approved) -> transaction_audit_events + users -> TransactionLedger merender loading/error/retry/history_available/timeline inline pada detail yang sama`; response selection lama diabaikan, row legacy tanpa event tetap jujur `history_available=false`, dan dialog hanya dipakai untuk mutation void.
...
- `src/views/admin/FinanceV2.vue` — host tipis route `/admin/finance`; menyediakan navigasi task-oriented dan `<RouterView>` tanpa summary/tab/workflow legacy.
- `src/views/admin/finance/TransactionsView.vue`, `DirectTransactionView.vue` (label UI **Catat kas**), `ProposalsView.vue`, `ApprovalsView.vue`, dan `AuditHistoryView.vue` — child route nyata per workflow (`transaksi`, `transaksi-langsung`, `proposal`, `persetujuan`, `riwayat-audit`) yang tetap mengonsumsi composable/service kas existing. Ringkasan periode berada di workflow transaksi sehingga child dashboard finance dihapus.
- `src/views/admin/finance/TransactionsView.vue` + `src/components/admin/finance/TransactionWorkspace.vue` — implementation canonical transaksi berbasis shadcn-vue/Reka: summary periodik, inline quick filters server-bound, responsive activity list, mobile/tablet Sheet, wide-desktop persistent dossier, timeline audit, dan void dialog tanpa nested modal. Route evaluasi `transaksi-alt` dan submenu alternatif telah dihapus setelah approval user.
- `src/views/admin/finance/FinanceLegacyView.vue` — salinan referensi parity behavior dari komposisi finance lama; tidak terdaftar sebagai route/menu dan bukan baseline visual.
- `src/components/admin/finance/TransactionLedger.vue` — checkpoint lama yang hanya dipertahankan karena masih dipanggil `FinanceLegacyView.vue`; bukan route transaksi canonical.
...
- `src/services/admin/kasService.ts` — `getMasterData`, `getTransactions`, `submitDirectTransaction`, `submitProposal`, `approveTransaction`, `voidTransaction`, `getTransactionTimeline` — API client transaksi kas; `approveTransaction(id, action, reason, key)` menangani action `approve|reject` melalui satu endpoint approval, sedangkan list memakai query periodik `month/year` + filter optional `tipe/kategori_id`.
...
- `src/components/admin/kas/DirectCashDesk.vue` — presentasi canonical route aktif Catat kas berbasis Entry Spine: semantic radio arus, nominal dominan, identitas transaksi, seksi progressive disclosure, closure review tanpa persistent dossier, dan Dialog final pending/retry-safe; mutation tetap dimiliki composable/service. `KasInput.vue` hanya legacy bridge dan bukan baseline active route.
- `src/components/admin/kas/KasProposal.vue` — task surface canonical Proposal berbasis Request Brief: keperluan menjadi content focus pertama, nominal berada di opening decision group, rincian tetap brief utama, metadata routing memuat consequence + CTA tunggal tanpa sticky, dan review/mutation final dimiliki Dialog pending/retry-safe; `seksi_id` wajib dan mutation tetap melalui `useKas.handleProposal`.
...
- `src/services/admin/mediaService.ts` — `uploadMedia`, `listMedia`, `deleteMedia`, `updateMediaMetadata` — API client media library admin (multipart upload, list pagination/filter, request safe-delete lifecycle, patch metadata).
...
- `server/api/admin/transaction.ts` — `GET master-data/list/pending/:id/timeline`, `POST add-direct/add-proposal/approve/reject/:id/void`; hard-delete legacy ditutup dengan `405` — endpoint transaksi kas dengan pemisahan intent, approval berjenjang ketua→bendahara, audit/void, idempotency, middleware auth/role, validasi, scoping data, dan filter periodik.
- `server/services/transaction.ts` — `createTransaction`, `getPendingTransactions`, `getAllTransactions`, `updateStatus`, `voidTransaction`, `getTransactionAuditTimeline` — operasi inti transaksi, state machine proposal, audit event atomic, idempotency, data scoping, dan filter periodik.
...
- Skema data inti (dari `server/db/schema.sql`):
  ...
  - `kas_masjid` (transaksi kas; `keperluan` sebagai judul arus uang dan `keterangan` sebagai uraian, FK ke `kategori_kas`, `periode`, `seksi_pengurus`, `users`, dengan status bertahap `pending_ketua|pending_bendahara|approved|rejected|void` + metadata audit).
...
- Lokasi migration/seed:
  ...
  - Migration purpose transaksi: `migrations/0018_add_transaction_purpose.sql` (kolom additive nullable + backfill exact `keperluan = keterangan` untuk row existing).
...
- Migration DDL dan seed dipisah; `npm run test:migrations` memverifikasi fresh/upgrade, preservasi akun terotasi dan kas legacy, index, audit table, serta FK aktif-bersih.
...
- Critical-flow D1 dan authenticated browser E2E mengotomasi proposal pengurus → approval ketua → approval bendahara, reject, concurrency, ownership, period-summary, recovery, dan cleanup fixture.
...
- Dokumentasi canonical arsitektur, deployment, backup, recovery, redesign, dan agent workflow tersedia; observability/day-2 operations residual tetap berada di roadmap P1.
