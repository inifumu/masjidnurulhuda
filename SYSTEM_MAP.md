...
- `[Vue] KeuanganKas/useKas.loadData -> [Vue] kasService.getMasterData + getTransactions(month,year,tipe,kategori_id) -> [Vue] httpClient (credentials include + error normalization) -> [Hono] GET /api/admin/transaction/master-data + /list?month=&year=&tipe=&kategori_id= -> [Service] getAllTransactions(user,period,filters) -> DB kas_masjid (filter periode by \`tanggal\` + optional \`tipe/kategori_id\`, role=pengurus: approved OR created_by=user.sub|id)`
...
- `[Vue] KeuanganKas/KasApproval (Status Proposal) -> [Vue] kasService.getTransactions (/list?month&year&tipe&kategori_id) -> [Hono] GET /api/admin/transaction/list -> [Service] getAllTransactions(user,period,filters) -> DB kas_masjid (scoped per role + default current month/year bila query kosong + filter optional tipe/kategori)`
