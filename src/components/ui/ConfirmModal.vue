...
<template>
  <TransitionRoot as="template" :show="isOpen">
    <Dialog
      ...
      @close="!props.pending && emit('close')"
    >
      <TransitionChild
        as="template"
            ...
            >
              ...
              >
                <button
                  ...
                  :class="[
                    'inline-flex w-full justify-center rounded-lg px-3 py-2 text-sm font-semibold text-white shadow-sm sm:ml-3 sm:w-auto transition-colors disabled:cursor-not-allowed disabled:opacity-60',
                    type === 'danger'
                  ...
                  @click="!props.pending && emit('confirm')"
                >
                  {{ props.pending ? "Memproses..." : confirmText || "Konfirmasi" }}
                </button>
                <button
                  type="button"
                  :disabled="props.pending"
                  class="mt-3 inline-flex w-full justify-center rounded-lg bg-white dark:bg-[#1e293b] px-3 py-2 text-sm font-semibold text-slate-900 dark:text-slate-200 shadow-sm ring-1 ring-inset ring-slate-300 dark:ring-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 sm:mt-0 sm:w-auto transition-colors disabled:cursor-not-allowed disabled:opacity-60"
                  @click="!props.pending && emit('close')"
                >
                  Batal
                </button>
              </div>
