...
<template>
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="grid gap-8 border-b border-border py-12 md:grid-cols-[minmax(13rem,0.65fr)_minmax(0,1.35fr)] md:py-16">
      <header>
        <div class="flex items-center gap-2 text-primary"><Clock3 class="size-5" aria-hidden="true" /><p class="text-sm font-semibold">Waktu salat hari ini</p></div>
        ...
        <p v-else-if="source === 'stale'" class="mt-3 text-xs font-medium text-destructive">Data cache lama — konfirmasi waktu sebelum berangkat.</p>
      </header>
        ...
        <template v-else>
          <div class="grid grid-cols-2 gap-px border-y bg-border sm:grid-cols-5">
            <div v-for="waktu in jadwal" :key="waktu.nama" class="bg-background px-3 py-5 last:col-span-2 sm:last:col-span-1">
              ...
              <p class="font-mono mt-2 text-2xl font-semibold tracking-tight">{{ waktu.waktu }}</p>
            </div>
          </div>
          <div v-if="source === 'stale'" class="mt-4 flex flex-wrap items-center justify-between gap-3 border-l-2 border-destructive bg-muted px-4 py-3 text-sm text-destructive">
