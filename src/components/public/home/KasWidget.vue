...
<template>
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="grid gap-10 py-16 md:grid-cols-[minmax(15rem,0.7fr)_minmax(0,1.3fr)] md:py-24">
      <header>
        <p class="text-xs font-semibold uppercase tracking-[0.08em] text-primary">Transparansi publik</p>
      ...
      <div>
        ...
        <dl v-else class="divide-y border-y">
          <div v-for="metric in metrics" :key="metric.key" class="grid gap-2 py-5 sm:grid-cols-[1fr_auto] sm:items-center">
            <dt class="flex items-center gap-2 text-sm font-medium text-muted-foreground"><component :is="metric.icon" class="size-4 text-primary" aria-hidden="true" />{{ metric.label }}</dt>
            <dd class="font-mono text-2xl font-semibold tracking-tight sm:text-3xl">{{ formatRupiah(kasSummary[metric.key]) }}</dd>
          </div>
        </dl>
