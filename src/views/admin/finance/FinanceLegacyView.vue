...
import TransactionLedger from "@/components/admin/finance/TransactionLedger.vue";
    ...
    <div class="bg-white/60 dark:bg-[#09090b]/60 backdrop-blur-xl rounded-xl border border-slate-200/60 dark:border-slate-800/60 shadow-sm flex flex-col">
      ...
      <div class="p-4 md:p-6 min-h-[400px]">
        ...
        <TransactionLedger
          ...
          :transactions="filteredLaporan" :categories="categories"
          :month="selectedMonth" :year="selectedYear" :years="availableYears"
          :flow="filterTipe" :category="filterKategori"
          :saldo-awal="globalSaldoAwal" :total-masuk="globalMasuk"
          :total-keluar="globalKeluar" :saldo-akhir="globalSaldoAkhir"
          :pending-ids="pendingMutationIds" :loading="isLoadingData" :format-rupiah="formatRupiah"
          :format-waktu-audit="formatWaktuAudit"
          @update:month="selectedMonth = $event" @update:year="selectedYear = $event"
          @update:flow="filterTipe = $event" @update:category="filterKategori = $event"
          @void="openVoidConfirm" @timeline="openTimeline"
        />
