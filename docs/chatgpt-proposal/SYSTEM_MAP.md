...
- `[Vue] LoginV2.vue(handleLogin) -> [Vue] authStore.login() -> [Hono] POST /api/admin/auth/login -> [Middleware] login rate limit baseline (IP+email, in-memory, 5 gagal/15 menit) -> [Hono] authService.loginAdmin -> [Query] getUserByEmail(users) + hash verify -> payload JWT (`tv`+`exp` 24 jam) -> set cookie httpOnly (maxAge 24 jam)`
- `[Vue] router.beforeEach -> [Vue] authStore.checkAuth() -> [Hono] GET /api/admin/auth/me -> verify JWT cookie + validasi claim `tv`vs`users.token_version`-> response user session (normalisasi logout hanya saat`401`; `5xx/network`diperlakukan error operasional tanpa force reset auth state; guard redirect login aktif untuk`/admin/login` saat sesi sudah authenticated)`
...
- `[Vue] PengaturanV2.vue -> [Vue] PengaturanLegacyBridge.vue -> [Vue] usePengaturan.loadData|saveItem|deleteItem -> [Vue] pengaturanService/httpClient -> [Hono] /api/admin/pengaturan/(kategori|seksi|users) -> service kategori|seksi|user -> DB kategori_kas(jenis_arus)|seksi_pengurus|users`
