...
- [~] Apply + verifikasi migration D1 remote production (`0003` s.d `0007`).
  ...
  - update Mei 2026 (role reconcile): `migrations/0007_reconcile_bendahara_role.sql` direvisi menjadi migration schema nyata untuk memastikan CHECK constraint role `users` mengizinkan `bendahara` (`superadmin|ketua|bendahara|pengurus`) dengan pola kompatibilitas D1 non-fresh (`PRAGMA foreign_keys=OFF` saat rebuild `users`, lalu `PRAGMA foreign_keys=ON` kembali), serta menyesuaikan struktur kolom auth ke `password_hash` (bukan `password`) agar kompatibel dengan baseline `0001`.
