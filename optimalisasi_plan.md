...
- [~] Apply + verifikasi migration D1 remote production (`0003` s.d `0007`).
  ...
  - update Mei 2026 (role reconcile): fokus diperketat ke **PENAMBAHAN role `bendahara`** pada DB live. Percobaan patch constraint via `sqlite_master` gagal di remote D1 dengan `SQLITE_AUTH (code: 7500)` karena operasi `writable_schema` tidak diizinkan. Strategi final `migrations/0007_reconcile_bendahara_role.sql` diubah ke jalur legal D1: rebuild tabel `users` (`CREATE users_new -> copy data -> DROP old -> RENAME`) dengan CHECK baru (`superadmin|ketua|bendahara|pengurus`) + normalisasi minimal `role IS NULL -> pengurus`, tanpa mapping role legacy spekulatif.
